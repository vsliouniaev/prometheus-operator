...
func (f *Framework) CreatePrometheusOperatorWithAdmissionHooks(ns, opImage string, namespacesToWatch []string) ([]finalizerFn, error) {

	var finalizers []finalizerFn

	certBytes, err := f.CreatePrometheusOperator(ns, opImage, namespacesToWatch)
	if err != nil {
		return nil, err
	}
...
func (f *Framework) CreatePrometheusOperator(ns, opImage string, namespacesToWatch []string) ([]byte, error) {
	tru := true
	fals := false
